...
On peut estimer la variance des estimateurs avec 
\begin{equation}
\hat{\mathrm{Var}}(\hat{\theta}) = I(\hat{\theta})^{-1}
\end{equation}


\subsection{Méthode delta}
On peut approximer la variance d'une fonction $g(\alpha, \theta)$ avec la formule suivante : 
\begin{align*}
\mathrm{Var}(g(\alpha, \theta)) & = \variance{\alpha} \left( \derivee{\alpha} g(\alpha, \theta) \right)^2 + \variance{\theta} \left( \derivee{\theta} g(\alpha, \theta ) \right)^2  \\
& + 2 \mathrm{Cov}(\alpha, \theta) \derivee{\alpha} g(\alpha, \theta) \derivee{\theta} g(\alpha, \theta) \\ \numberthis
\end{align*}
...
% Pas vu en classe ... à valider si dans le syllabus pour l'exam final
